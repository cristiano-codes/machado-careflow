...
async function main() {
  ...
  try {
    ...
    pushCheck("legenda de cores visivel", pageText.includes("legenda de cores"), null);
    pushCheck(
      "painel pendentes da triagem visivel",
      pageText.includes("pendentes de agendamento"),
      null
    );
    ...
    pushCheck(
      ...
      (await page.content()).toLowerCase().includes("detalhes do agendamento"),
      null
    );
    ...
    const confirmResponsePromise = page
      ...
      .catch(() => null);
    const confirmClicked = await clickButtonByText(page, "Confirmar");
    ...
    const cancelResponsePromise = page
      ...
      .catch(() => null);
    const cancelClicked = await clickButtonByText(page, "Cancelar");
