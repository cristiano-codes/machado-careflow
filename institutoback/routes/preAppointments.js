...
// GET - Fila operacional de triagem do pre-agendamento
router.get('/triage-queue', authMiddleware, authorizePreAppointmentsLookup, async (req, res) => {
  ...
  try {
    ...
    console.error('Erro ao listar fila de triagem de pre-agendamento:', error);
    return res.status(500).json({
      success: false,
      message: 'Erro interno do servidor',
    });
...
// PATCH - Atualizar status/observacao operacional da fila de pre-agendamento
router.patch('/:id/triage', authMiddleware, authorizePreAppointmentsLookup, async (req, res) => {
  ...
  try {
    ...
    if (currentOperationalStatus === 'converted' && requestedStatus) {
      ...
      return res.status(409).json({
        success: false,
        message: 'Pre-agendamento convertido nao pode voltar para status de triagem.',
      });
    }
    ...
    return res.json({
      success: true,
      preAppointment: mapPreAppointmentRow(updateResult.rows[0]),
    });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao atualizar triagem de pre-agendamento:', error);
    return res.status(500).json({
      success: false,
      message: 'Erro interno do servidor',
    });
